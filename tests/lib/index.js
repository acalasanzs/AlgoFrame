...
exports.Timeline = exports.Animate = void 0;
...
class Animate {
    constructor(starttime = 0, preset, keyframes, FPS = undefined, loop = false) {
        this.starttime = starttime;
        this.preset = preset;
        this.keyframes = keyframes;
        // * TIMELINE FEATURES
        this._running = [];
        this.preset = (0, utils_1.passPreset)(preset);
    ...
    set FPS(value) {
        ...
        if (FPS) {
            ...
            console.warn(new Error('Not a valid Number'));
    }
    nextTime(arr = this._running) {
        if (!arr.length) {
            console.log(new Error());
        }
        return arr.reduce((previousValue, currentValue) => currentValue.time < previousValue.time ? currentValue : previousValue);
    }
    ...
    timeline(array, real, reverseLoop) {
        ...
        array.forEach(event => {
            ...
            data.push({
                _: new AlgoFrame(event.duration, event.delay ? event.delay : 0, event.easing ? event.easing : this.easing, event.keyframes ? event.keyframes.clone() : this.keyframes.clone(), this._FPS).finally(event.finally),
                time: event.time,
                callback: event.run,
            });
        });
        ...
        let last = this._timeline.reduce((previousValue, currentValue) => currentValue.time > previousValue.time ? currentValue : previousValue);
        this.reversed = false;
        this.callback = function (X, easedProgress, params) {
            ...
            const next = () => {
                ...
                this._running.shift();
                console.log(this._next.time, this._running.map(l => l.time));
            ...
            if (this._next) {
                ...
                first = this._next;
                last = this._timeline.reduce((previousValue, currentValue) => currentValue.time > previousValue.time ? currentValue : previousValue);
                this._running.shift();
                this._next = this.nextTime();
                next();
            }
    ...
    run(callback, precision = this._FPS) {
        ...
        const animate = (timestamp) => {
            ...
            if (condition) {
                ...
                sent = true;
                this.callback(this.waiting ? 0 : this.keyframes.test(Math.min(easedProgress, 1)), Math.min(easedProgress, 1), {
            ...
            if (!this.stop) {
                if (runtime < this.duration) {
                    ...
                    if (this.loop)
                        requestAnimationFrame(animate.bind(this));
                    (_a = this.next) === null || _a === void 0 ? void 0 : _a.call(this);
                }
                else {
                    this.done = true;
                    this.keyframes.restart();
                    if (this.loop)
                        requestAnimationFrame(animate.bind(this));
                    (_b = this.next) === null || _b === void 0 ? void 0 : _b.call(this);
                }
            }
